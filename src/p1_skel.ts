    ...
    override configure(newState : string) {
        ...
        switch (this.currentState) {
            ...
            case 'begin_trial':
                ...
                // === YOUR CODE HERE ===
                this.theBackground.msg1 = "Trial #" + this.trialCount + " of " + this.MAX_TRIALS; // display trial #
                this.theBackground.msg2 = "";
                this.theBackground.msg3 = "";
...
class Target extends ScreenObject{
    ...
    newGeom(newCentX : number, newCentY : number, newDiam? : number) {
        ...
        // === YOUR CODE HERE ===
        // deal with optional diameter param
    ...
    override draw(ctx : CanvasRenderingContext2D) : void {
        ...
        ctx.fill();
        ctx.lineWidth = 1; // set as default
    ...
    override pickedBy(ptX : number, ptY : number) : boolean {
        ...
        // use the distance from point to center to check if within the circle
    ...
    override handleClickAt(ptX : number, ptY : number) : boolean {
        ...
        // === YOUR CODE HERE ===
        if (!this.pickedBy(ptX, ptY)) return false;

        this.parentUI.recordTrialEnd(ptX, ptY, this._w);
...
class Reticle extends Target {
    ...
    constructor(centerX : number, centerY : number, parent : FittsTestUI) {
        ...
        super(centerX,centerY,Reticle.RETICLE_DIAM, parent);
    ...
    override draw(ctx : CanvasRenderingContext2D) : void {
        ...
        // draw vertical line
        ctx.beginPath();
        ctx.moveTo(this.centerX, this.centerY - outerRadius);
        ctx.lineTo(this.centerX, this.centerY + outerRadius);
        ctx.stroke();

        // draw horizontal line
        ctx.beginPath();
        ctx.moveTo(this.centerX - outerRadius, this.centerY);
    ...
    override pickedBy(ptX : number, ptY : number) : boolean {
        ...
        // use distance from point to center of reticle to check if within
        const dx = ptX - this.centerX;
        const dy = ptY - this.centerY;
        const dist = Math.sqrt(dx * dx + dy * dy);
...
class BackgroundDisplay extends ScreenObject{
    ...
    override draw(ctx : CanvasRenderingContext2D) : void {
        ...
        ypos += (fontHeight + leading); // move to next line
    ...
    override handleClickAt(ptX : number, ptY : number) : boolean {
        ...
        // === YOUR CODE HERE ===
        if (!(this.parentUI.currentState === 'start')) return false;
